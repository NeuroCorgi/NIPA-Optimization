...
def get_args():
    ...
    parser.add_argument(
        '--compensate_fluctuations', action='store_true', default=True,
        help='Compensate for fluctuations in the data where each weekend has lower reported cases (default: True)')
    parser.add_argument(
        '--no_compensate_fluctuations', action='store_false', dest='compensate_fluctuations',
        help='Do not compensate for fluctuations in the data where each weekend has lower reported cases')
    parser.add_argument(
        '--predict', action='store_true',
        help='Only predict the next days based on the trained model')
    parser.add_argument(
        '--no_predict', action='store_false', dest='predict',
        help='Do not predict the next days based on the trained model')
    parser.add_argument(
        '--random', action='store_true',
        help='Randomize the seed for LASSO and simulated annealing used for the NIPA (otherwise random_state=42 is used)')
    parser.add_argument(
        '--no_random', action='store_false', dest='random',
        help='Do not randomize the seed for LASSO and simulated annealing used for the NIPA (use random_state=42 instead)')
...
def predict(I_data, regions, dates_list, settings, final_dates, visualisations=[Visualizations.ALL], save=False):
    ...
    for idx, pred_days in enumerate(n_pred_days):
        ...
        try:
            for optimizer in optimizers:
                B.append(pl.read_csv(join(io.get_results_data_path(country, type, optimizer, data_date), "B.csv")))
                curing_probs.append(pl.read_csv(join(io.get_results_data_path(country, type, optimizer, data_date), "curing_probs.csv")))
