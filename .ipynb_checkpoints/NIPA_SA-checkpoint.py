...
class NIPA:
    ...
    def train(self, train_days):
        ...
        for i in self.regions.keys():
            ...
            print(f'P min: {p_min_max[0]} P max: {p_min_max[1]}')
            print(f'Regularization value: {p_value}')
            print(f'evaluation: {evaluation_region}')
            print(f'B: {B_region}')
            print()
    ...
    def calc_initial_temp(self, q_a, initial_acceptance_prob):
        with open('data/avg_E_all.txt', 'r') as f:
            lines = f.readlines()
            avg = np.average([float(line) for line in lines if float(line) != 0.0])

            print(f"Average E: {avg}")
    ...
    def calc_k(self, T0, initial_acceptance_prob):
        with open('data/std_E.txt', 'r') as f:
            lines = f.readlines()
            all_E = [float(line) for line in lines]
            
        avg_std = np.average(all_E)
        
        k = -(T0 * np.log(initial_acceptance_prob)) / avg_std
    ...
    def inference(self, I_region, I_df):
        ...
        # SECOND BEST: 15.326 * 0.04578861788617888
        q_a = -5
        initial_acceptance_prob = 0.8
        T0 = self.calc_initial_temp(q_a, initial_acceptance_prob)
        k = self.calc_k(T0, initial_acceptance_prob)
        temp = k * T0

        print(f"T0 = {T0}; K = {k}; Initial Temperature = {temp}")
        ...
        print(f"Curing prob: {best_curing_prob}")
